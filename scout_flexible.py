"""
Scout for accepting input as CAS number or element name and perfrom flexible validation prcocess
"""
...
# Dictionaries to keep track of visit counts for URLs and domains
URL_VISIT_COUNT = {}
DOMAIN_VISIT_COUNT = {}
MAX_URL_VISITS = 5
MAX_DOMAIN_VISITS = 5
...
def save_report():
    """
    Save the report list to a JSON file in the logs directory.
    """
    if REPORT_LIST:
        try:
...
# Add report entry
def add_report(cas, name, filename, downloaded, provider):
    """
    Add an entry to the report list.

    Params:
        cas (str): CAS number of the chemical.
        name (str): Name of the chemical.
        filename (str): Filename of the downloaded PDF.
        downloaded (bool): Whether the file was successfully downloaded.
        provider (str): Provider of the PDF file.
    """
    report = {"cas": cas, "name": name, "provider": provider, "downloaded": downloaded, "filename": filename}
...
def download_pdf(url, folder_path):
    """
    Download the PDF file from the given URL to the specified folder.

    Params:
        url (str): URL of the PDF file.
        folder_path (str): Folder path to save the downloaded PDF.
...
def extract_text_from_pdf(pdf_path):
    """
    Extract text content from the specified PDF file.
...
def set_pattern(sequence):
    """
    Create a compiled regular expression pattern for the given sequence.

    Params:
        sequence (str): Sequence to compile into a regex pattern.
...
def verify_pdf(file_path, cas=None, name=None):
    """
    Verify if the PDF file contains the specified CAS number or name and the term "safety data sheet".

    Params:
        file_path (str): Path to the PDF file.
        cas (str): CAS number to search for (optional).
        name (str): Chemical name to search for (optional).

    Returns:
        bool: True if the PDF contains the specified terms, False otherwise.
    """
    ...
    if cas:
        ...
        pattern1 = re.compile('|'.join(map(re.escape, name.split())), re.IGNORECASE)
...
def rename_and_move_file(file_path, destination, cas_or_name, provider):
    ...
    Params:
        ...
        cas_or_name (str): CAS number or name to include in the new file name.
        provider (str): Provider name to include in the new file name.

    Returns:
        str: New file name, or None if an error occurred.
    """
    try:
...
def scrape_urls(url, base_url, timeout=10):
    """
    Scrape all URLs from the given webpage.
...
def find_pdfs(url, depth=2, base_url=None, cas=None, name=None):
    """
    Recursively find PDF files from the given URL up to the specified depth.

    Params:
        url (str): Starting URL for finding PDFs.
        depth (int): Maximum depth for recursive search.
        base_url (str): Base URL for resolving relative links.
        cas (str): CAS number for verification (optional).
        name (str): Chemical name for verification (optional).
    """
    ...
    if is_pdf(url):
        ...
        if file_path:
            if verify_pdf(file_path, cas, name):
                ...
                file_name = rename_and_move_file(file_path, PDFS_FOLDER, name or cas, provider_name)
                if file_name:
                    add_report(cas, name, file_name, True, provider_name)
            else:
                print(f"Verification status: {file_path} is not a verified MSDS")
                # Move to temp folder for manual verification
                temp_file_name = rename_and_move_file(file_path, TEMP_FOLDER, name or cas, "temp")
                if temp_file_name:
                    add_report(cas, name, temp_file_name, False, "temp")
    else:
...
def scout(cas_or_name, max_search_results=10):
    """
    Search Google for MSDS files and initiate the PDF finding process.

    Params:
        cas_or_name (str): CAS number or chemical name to search for.
